...
FOpaqueVSOutput VS_Opaque(VS_Input_PNCT_T Input)
{
    ...
    // Gouraud Shading???•ì  ?¼ì´??ê³„ì‚°???„í•´ ?”ë“œ ?¬ì???ê³„ì‚°
    // float4(pos, 1.0f)ë¡?w=1??ëª…ì‹œ?´ì•¼ Model ?‰ë ¬???´ë™ ?±ë¶„???ìš©??(??ê·¸ëŸ´ ??w=0 ?˜ë©° ? ì•„ê°?
...
FOpaqueOutput2 PS_Opaque_Gouraud(FOpaqueVSOutput Input)
{
    ...
    // ?•ì ?ì„œ ê³„ì‚°???¼ì´??ê°’ì„ ê·¸ë?ë¡?G-Buffer(Surface1)??ê¸°ë¡
...
FOpaqueOutput3 PS_Opaque_BlinnPhong(FOpaqueVSOutput Input)
{
    ...
    // SpecularStrengthë¥?0.3?¼ë¡œ ??¶°???˜ì´?¼ì´?¸ê? ?˜ì–—ê²??€ë²„ë¦¬???„ìƒ??ë°©ì?

...
void FD3DDevice::OnResizeViewport(int Width, int Height)
{
    ...
    // ?íƒœ ìºì‹œ ì´ˆê¸°?????ˆë¡œ ?ì„±??state ê°ì²´ê°€ BeginFrame?ì„œ ?¬ì ?©ë˜?„ë¡
...
void FD3DDevice::CreateDeviceAndSwapChain(HWND InHWindow)
{
    ...
    D3D11CreateDeviceAndSwapChain(nullptr, D3D_DRIVER_TYPE_HARDWARE, nullptr,
                                  CreateDeviceFlags, featureLevels, ARRAY_SIZE(featureLevels), D3D11_SDK_VERSION,
    ...
    // CPUê°€ GPUë³´ë‹¤ 1?„ë ˆ???´ìƒ ?žì„œì§€ ëª»í•˜ê²??œí•œ
    // (ê¸°ë³¸ê°?3 ??Present ??ê¹Šì´ë¡??¸í•œ FPS ?±ë‹ˆ???„ìƒ ë°©ì?)
    {
...
#ifdef _DEBUG
    // ë¦?ì§„ë‹¨: Device ?´ì œ ì§ì „ ?¨ì•„?ˆëŠ” D3D ê°ì²´ ë¦¬í¬??
    if (Device)
    {

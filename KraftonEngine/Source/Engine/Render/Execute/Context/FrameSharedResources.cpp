...
void FFrameRenderResources::Create(ID3D11Device* InDevice)
{
    ...
    // s0: LinearClamp (PostProcess, UI, ê¸°ë³¸)
    {
...
void FFrameRenderResources::UpdateLocalLights(ID3D11Device* Device, ID3D11DeviceContext* Context, const TArray<FLocalLightInfo>& Lights)
{
    ...
    if (Count > LocalLightCapacity)
    {
        ...
        {
            // ?ëŸ¬ ì²˜ë¦¬ (ë¡œê·¸ ì¶œë ¥, assert ??
            MessageBox(nullptr, TEXT("LocalLightBuffer ?ì„± ?¤íŒ¨"), TEXT("Error"), MB_OK | MB_ICONERROR);
            return;
        }
        ...
        if (FAILED(hr))
        {
            MessageBox(nullptr, TEXT("LocalLightSRV ?ì„± ?¤íŒ¨"), TEXT("Error"), MB_OK | MB_ICONERROR);
        }

...
public:
	...
	// ViewportType + GizmoMode ??AxisMask Í≥ÑÏÇ∞ (Proxy?êÏÑú???¨Ïö©)
	...
	// Actor ?ÜÏù¥ ?ÖÎ¶Ω ?ùÏÑ±??Gizmo?????∏Î??êÏÑú Scene??ÏßÅÏ†ë ÏßÄ??
...
private:
	...
	static constexpr float GizmoScreenScale = 0.15f; // ?îÎ©¥ ?ÄÎπ?Í∏∞Ï¶àÎ™??¨Í∏∞ ÎπÑÏú®
	...
	uint32 AxisMask = 0x7; // ÎπÑÌä∏ 0=X, 1=Y, 2=Z ??LineTrace??(?åÎçîÎßÅÏ? ProxyÍ∞Ä ÏßÅÏ†ë Í≥ÑÏÇ∞)
	FPrimitiveSceneProxy* InnerProxy = nullptr;	// GizmoInner ?ÑÏö© ?ÑÎ°ù??
	FScene* RegisteredScene = nullptr;			// Actor ?ÜÏù¥ ?ÖÎ¶Ω ?ùÏÑ± ???¨Ïö©
};
